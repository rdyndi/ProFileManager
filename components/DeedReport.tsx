...
export const DeedReport: React.FC<DeedReportProps> = ({ deeds, onBack }) => {
  ...
  const handlePrint = () => {
    ...
    const rowsHtml = filteredDeeds.map((deed, index) => {
        ...
        return `
            <tr style="border-bottom: 1px solid #000; vertical-align: top;">
                <td style="padding: 4px; text-align: center; border-right: 1px solid #000; word-break: break-word;">${deed.orderNumber}</td>
                <td style="padding: 4px; text-align: center; border-right: 1px solid #000; word-break: break-word;">${index + 1}</td>
                <td style="padding: 4px; text-align: center; border-right: 1px solid #000; word-break: break-word;">
                    ${new Date(deed.deedDate).toLocaleDateString('id-ID', {day: '2-digit', month: 'long', year: 'numeric'})}
                </td>
                <td style="padding: 4px; text-align: left; border-right: 1px solid #000; font-weight: 500; line-height: 1.2; word-break: break-word;">
                    ${deed.deedTitle}
                </td>
                <td style="padding: 4px; text-align: left; line-height: 1.2; word-break: break-word;">
                    ${appearersHtml}
                </td>
            </tr>
        `;
    ...
    const emptyRows = filteredDeeds.length === 0 ? `
        <tr><td colspan="5" style="text-align: center; padding: 20px; font-style: italic; color: #555; border-bottom: 1px solid #000;">Tidak ada data akta pada bulan ini.</td></tr>
    ` : '';
    ...
    element.innerHTML = `
        <div style="font-family: 'Inter', sans-serif; padding: 20px; color: #000; width: 100%;">
            ...
            <div style="margin-bottom: 20px;">
                <h1 style="font-size: 14px; font-weight: bold; text-transform: uppercase; margin-bottom: 5px;">Salinan Daftar Akta-Akta Notaris ${signatureName}</h1>
                <h2 style="font-size: 12px; font-weight: bold; text-transform: uppercase;">Bulan ${monthName} ${selectedYear}</h2>
            </div>

            <table style="width: 100%; border: 1px solid #000; border-collapse: collapse; font-size: 10px; table-layout: fixed;">
                <thead>
                    <tr style="border-bottom: 1px solid #000; background-color: #f3f4f6; text-transform: uppercase; font-weight: bold; text-align: center;">
                        <th style="padding: 4px; border-right: 1px solid #000; width: 8%; word-break: break-word;">NOMOR URUT</th>
                        <th style="padding: 4px; border-right: 1px solid #000; width: 10%; word-break: break-word;">NOMOR BULANAN</th>
                        <th style="padding: 4px; border-right: 1px solid #000; width: 12%; word-break: break-word;">TANGGAL</th>
                        <th style="padding: 4px; border-right: 1px solid #000; width: 35%; word-break: break-word;">SIFAT AKTA</th>
                        <th style="padding: 4px; width: 35%; word-break: break-word;">NAMA PENGHADAP</th>
                    </tr>
                </thead>
                <tbody>
                    ${rowsHtml}
            ...
            <div style="margin-top: 30px; display: flex; justify-content: flex-end; page-break-inside: avoid;">
                 <div style="text-align: center; width: 50%;">
                    <p style="font-size: 12px; margin-bottom: 15px; line-height: 1.5; font-weight: 500;">Salinan Daftar Akta-Akta yang telah dibuat oleh saya, Notaris, selama Bulan ${monthName} ${selectedYear}.</p>
    ...
    const opt = {
        margin: 10,
        filename: `Laporan_Akta_Bulanan_${monthName}_${selectedYear}.pdf`,
